<?php
    ...
    $plantilla='DTAJOVWD';                          // <-- Clase de Plantilla [Adulto Joven]
    ...
    // UNION ALL
    // select CAMPO, DESCCAMPO from mpld where CLASEPLANTILLA='$plantilla' and ltrim(rtrim(tipo_campo)) not in ('Titulo','Subtitulo')";

    $cabecera = $cabecerag . "
    UNION ALL
    select CAMPO, DESCCAMPO from mpld where CLASEPLANTILLA='$plantilla' and ltrim(rtrim(tipo_campo)) not in ('Titulo','Subtitulo')";

    // SE ARMA LA CONSULTA PARA CONCATENAR CAMPOS EN EL LLAMADO AL STORED PROCEDURE

    $qry = "SELECT CAMPO, DESCCAMPO FROM mpld WHERE CLASEPLANTILLA='$plantilla' and ltrim(rtrim(tipo_campo)) not in ('Titulo','Subtitulo')";
